...
function PostcssCompiler(inputTrees, inputFile, outputFile, plugins, map, useSass, includePaths) {
...
PostcssCompiler.prototype.updateCache = function(includePaths, destDir) {
    ...
    //Sass compiling first!
    if(this.useSass) {
		var importPath = path.join(__dirname, "../..", "app/styles");

		var incPaths = this.includePaths.concat(importPath);
...
PostCSSPlugin.prototype.toTree = function(tree, inputPath, outputPath, inputOptions) {
	...
	var trees = Object.keys(options.outputPaths).reduce(function(trees, file) {
		...
		if (input) {
			trees.push(new PostcssCompiler(inputTrees, input, output, options.plugins, options.map, options.useSass, options.includePaths));
		}
		return trees;
	}, []);
